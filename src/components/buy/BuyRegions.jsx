...
const REGIONS = [
  {
    name: "Halifax",
    image:
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=1600",
  },
  {
    name: "Dartmouth",
    image:
  ...
  },
  {
    name: "Lunenburg",
    image:
      "https://images.unsplash.com/photo-1600585154084-4e5fe7c39198?w=1600",
  },
  {
    name: "Chester",
    image:
...
export default function BuyRegions({ onSelectCity }) {
  ...
  const handleCitySelect = (cityName) => {
    if (onSelectCity) {
      ...
      router.push(`/buy/${cityName.toLowerCase().replace(/\s+/g, "-")}`);
    }
  };
        ...
        <div className="lg:hidden -mx-6 px-6">
          <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
            {REGIONS.map((region) => (
              <button
                key={region.name}
                onClick={() => handleCitySelect(region.name)}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {REGIONS.map((region) => (
              ...
              >
                ...
                <div
                  className="pointer-events-none absolute inset-0 bg-gradient-to-br from-red-50 via-white to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                />
