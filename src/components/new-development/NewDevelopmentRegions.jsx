...
export default function NewDevelopmentRegions() {
  ...
  useEffect(() => {
    // Fetch new development counts specifically
    fetch("/api/bridge/new-development-counts")
      .then((res) => {
        if (!res.ok) {
          console.warn("Failed to fetch new development counts, using empty object");
          return {};
        }
        return res.json();
      })
      .then((data) => {
        console.log("📊 New Development counts received:", data);
        setCounts(data);
      })
      .catch((err) => {
        console.error("Error fetching new development counts:", err);
        setCounts({});
      });
  }, []);
      ...
      <div className="relative max-w-[1600px] mx-auto px-6 grid lg:grid-cols-2 gap-16">
        ...
        <div>
          ...
          <p className="mt-6 text-lg text-gray-600 max-w-md">
            Explore new developments and construction projects across Nova Scotia's most desirable communities.
          </p>
        </div>
